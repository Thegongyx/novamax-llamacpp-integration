...
### 2. Kairic Edge（`rocm_ciru` 引擎）模型的必要参数
...
`rocm_ciru` 引擎跑 `Qwen3.8-27B-IU4-Kairic-Edge` 这类**带 PromptForge 侧车**的模型，除了常规
llama-server 参数外，还必须**用环境变量**指定 3 个 `.pfs` 侧车（llama-server 启动时读取，
**不是** CLI 参数；CLI 只用来传 `--kairic-edge` 等开关）。

#### 2.1 环境变量（用户级，必须重启 NovaMax 后端）
...
# 直接控制贪心快速路径的是 LLAMA_TARGET_GREEDY_ARGMAX_FASTPATH（服务器读取，1=开/0=关）。
# 注意：KAIRIC_EDGE_COMPATIBILITY_MODE 只在 shell 启动脚本 run-kairic-edge-gfx1151.sh 里
# 被用来换算上面这个变量，NovaMax 直接 spawu llama-server 时服务器【不读】它，设了也无用。
```

> 设置后必须**重启 NovaMax 后端**，llama-server 子进程才继承这些环境变量。

#### 2.2 NovaMax 模型 `user_parameters` 键值

NovaMax 把 `user_parameters` 的键拼成 `--<key> <value>`；**布尔 `true` → 裸标志**
（如 `kairic-edge: true` → `--kairic-edge`，不带值）。
...
#### 2.3 关闭贪心快速（接 agent 必读）

`LLAMA_TARGET_GREEDY_ARGMAX_FASTPATH` 控制 target 模型的**贪心 argmax 快速路径**（服务器在
`tools/server/server-task.cpp` 读取）：

- **`=1`（快速）**：只接受**纯贪心**请求——`temperature=0 / top_k=0/1 / top_p=1 / min_p=0`，且**不能带
  `logprobs/概率`、`grammar`（工具调用）、`logit_bias`、`LoRA`、`reasoning budget`、多补全、惩罚**。
...
- **`=0`（兼容模式）**：关闭 target 贪心快速路径，**放行采样 / 概率 / 工具调用 / grammar / logit_bias** 等
  正常请求。代价是不再走贪心 argmax 快速路径（略慢，约 -10%）。

> **结论：大多数 agent（如 Hermes）都发工具调用/采样/概率请求 → 必须用 `=0` 兼容模式。** 本包环境变量
> 块默认已设 `0`。若你只跑纯 chat、确要贪心最快，再改成 `1` 并确保请求为纯贪心。

```powershell
# 关闭贪心快速（接 agent）：用户级，设置后重启 NovaMax 后端
[Environment]::SetEnvironmentVariable('LLAMA_TARGET_GREEDY_ARGMAX_FASTPATH', '0', 'User')
```

> ⚠️ 真正控制它的是 `LLAMA_TARGET_GREEDY_ARGMAX_FASTPATH`（服务器直接读）；`KAIRIC_EDGE_COMPATIBILITY_MODE`
> 只被 shell 启动脚本 `run-kairic-edge-gfx1151.sh` 用来换算上面那个变量，**NovaMax 直接 spawn llama-server
> 时服务器不读它**，设了也无用。

#### 2.4 其他注意

- **别加 `kv-unified`**（`--kv-unified`）：Kairic/MTP 场景与 KV 池统一不兼容，缓存会反复失效（全量 re-prefill）。
- **若启动即报 `PromptForge initialization failed`**：多半是环境变量漏设/不对。
  核对 `PROMPTFORGE_GDN_IU4_HADAMARD=1`、`PROMPTFORGE_FFN_KEEPERS=late6`、`PROMPTFORGE_IU4_SEGMENTED=0`
  这些是否都已设置（缺失会触发 `...requires the nonsegmented Hadamard-IU4 FFN + Hadamard-IU4 GDN...`）。
- **启动日志必须出现**：`Kairic Edge: enabled (ngram=24/64/64, M65 verifier=strict-compact)` —— 有这行才算侧车加载成功。
- 若某键 NovaMax 不识别/拼错，用模型配置的"额外/自定义参数"字段写成完整 `--key value` 补进去。
